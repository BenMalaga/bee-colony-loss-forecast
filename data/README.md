...
> Integrity gate: the NASS pull fetches **outcome** data (colony loss). Per
> `../PRE_REGISTRATION.md`, don't fetch it until the pre-registration is committed. `fetch_nass.py`
...
- Grab a free API key at that page (it's instant). Then:
  ```bash
...
- Discovery: the fetch writes `data/raw/bee_colony_short_descs.txt` first, listing the exact
  `short_desc` strings, so the panel mapping (`src/build_panel.py`) can be pinned to real fields
  rather than guessed.
...
- Code plus `bee_data.csv` (NASS + PRISM weather + USDA Cropland Data Layer, 44 states, quarterly
  ...
  ```
- Used for two things: to cross-check my NASS panel against theirs, and to supply the weather and
  land covariates for H3.
- Respect their repo license; cite the paper (`docs/related_work.md`).

## 3. Context only (NOT modeled)

- FAO global colony series (the "managed bees are rising" context): <https://www.fao.org/faostat>
- Bee Informed Partnership Loss & Management survey (a different instrument; used descriptively
  only): <https://research.beeinformed.org/loss-map/>

Don't conflate BIP loss rates or FAO stock with the NASS state×quarter target. They measure
different things.
